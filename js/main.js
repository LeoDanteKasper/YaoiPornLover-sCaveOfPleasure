/**
 * ============================================================================
 * ENGINE NEXUS MULTIVERSE - CORE SYSTEM (v2.0.0)
 * ============================================================================
 * Código refactorizado, modular y libre de dependencias rotas.
 * Soporta carga dinámica de diálogos vía async/await, motor SFX WebAudio,
 * estado Sims enriquecido y renderizado de mapa dinámico en HTML5 Canvas.
 */

'use strict';

// ============================================================================
// 1. SISTEMA DE CARGA DINÁMICA DE PERSONAJES
// ============================================================================
window.dialogosDB = window.dialogosDB || {};

/**
 * Carga dinámicamente el archivo script de un personaje si no ha sido importado aún.
 * @param {string} nombre - Clave identificadora del personaje (ej: 'kaito', 'batman')
...
async function cargarPersonaje(nombre) {
    ...
    return new Promise((resolve, reject) => {
        ...
        script.onload = () => {
            console.log(`[System] Diálogos de '${key}' cargados correctamente.`);
            resolve();
        };
        
        script.onerror = () => {
            console.warn(`[System] No se pudo cargar el archivo js/${key}.js. Usando respuestas por defecto.`);
            reject(new Error(`No se pudo cargar el script de ${key}`));
        };
...
// ============================================================================
// 2. CHIPTUNE AUDIO ENGINE (Web Audio API)
// ============================================================================
...
const rawNpcsDB = {
    ...
    dark_pit: { name: "Pit Sombrío (Dark Pit)", desc: "El clon oscuro de Pit, con alas de pluma negra y una mirada desafiante.", room: "universo_nintendo", tags: ["edgy", "rebelde", "orgulloso", "antiheroe"], favoritePlace: "azotea_gothica" },
...
const citasDB = {
    lugares: {
        ...
        bar_subterraneo: { name: "Bar Speakeasy 'El Escondite'", reqAffinity: 15, boost: 25, category: "Noche / Fiesta", desc: "Música de jazz suave, tragos fuertes y una iluminación tenue." },
        ...
        azotea_gothica: { name: "Azotea de Rascacielos al Llover", reqAffinity: 20, boost: 35, category: "Romance / Vistas", desc: "Gárgolas de piedra mojadas y vista a la ciudad nocturna bajo una suave lluvia." },
...
// ============================================================================
// 4. MOTOR SIMS Y GESTIÓN DE PERSONAJES
// ============================================================================
class SimsEngine {
...
// Inicialización de la BD de NPCs compilada
const npcsDB = {};
for (let key in rawNpcsDB) {
    ...
    npcsDB[key] = {
        ...
        responses: {
            hablar: {
                low: `"Mira a su alrededor con cautela."`,
                high: `"Te habla con verdadera confianza."`
            },
            coquetear: {
                low: `"Se muestra algo tímido y sorprendido ante tu actitud."`,
                high: `"Sonríe de lado y te devuelve un cumplido atrevido."`
            }
        }
    };
}
...
// ============================================================================
// 5. BASE DE DATOS DE ESCENARIOS (ROOMS)
// ============================================================================
const roomsDB = {
    salon_central: {
...
// ============================================================================
// 6. ESTADO DE JUEGO Y CONTROL DE INTERFAZ
// ============================================================================
const gameState = {
...
function updateStats() {
    ...
    const elAfecto = document.getElementById('stat-afecto');
    if (elTurnos) elTurnos.textContent = gameState.turns;
    ...
    if (elAfecto) elAfecto.textContent = `${avg}%`;
}
...
// ============================================================================
// 7. PARSER DE COMANDOS Y ACCIONES DINÁMICAS CON IA / SCRIPTING
// ============================================================================
...
async function parseCommand(raw) {
    ...
    if (verb === "savegame") {
        ...
        await handleNPCActionAsync(target, act);
...
/**
 * Procesa la acción social importando dinámicamente los diálogos del archivo (nombre).js
 */
async function handleNPCActionAsync(targetName, action) {
    if (!targetName) {
    ...
    // Carga asíncrona del script del personaje si existe
    try {
        await cargarPersonaje(foundKey);
    } catch (err) {
        // Fallback silencioso
    }
    ...
    // 1. Prioridad: Buscar en window.dialogosDB
    if (window.dialogosDB && window.dialogosDB[foundKey] && window.dialogosDB[foundKey][action]) {
        const dialogObj = window.dialogosDB[foundKey][action];
        resp = (npc.affinity >= 30) ? (dialogObj.high || dialogObj.low) : dialogObj.low;
    } 
    // 2. Fallback: Buscar en la estructura local
    else if (npc.responses && npc.responses[action]) {
        resp = npc.responses[action];
    ...
    // 3. Fallback genérico
        ...
        resp = `"${npc.name} reacciona con sorpresa y curiosidad a tu acción."`;
    }
...
// ============================================================================
// 8. FUNCIONES AUXILIARES DE NAVEGACIÓN, CITAS Y HUD
// ============================================================================
...
function handleCita(targetName, lugarKey) {
    ...
    let bonus = (npc.favoritePlace === cleanLugarKey) ? 20 : 0;
    let totalBoost = lugar.boost + bonus;
    simsEngine.updateAffection(foundKey, totalBoost);
    updateStats();
    sfx.playAffectionUp();

    let reactionText = bonus > 0 
        ? `¡${npc.name} SE EMOCIONA ENORMEMENTE! Este es su lugar preferido de todo el multiverso. ¡Obtienes un bonus de +${bonus} pts!` 
        : `${npc.name} disfruta enormemente del ambiente relajado y conversan plácidamente durante horas.`;
...
function handleDuel(targetName) {
 ...
 ( o.o )  [SISTEMA DE COMBATE CHIPTUNE ACTIVO]
  > ^ <   ${npc.name} acepta el combate amigable.
            ...
            <p>Intercambian ráfagas de energía y movimientos tácticos. ¡Tras un duelo espectacular, quedan en empate!</p>
...
function handleGive(fullInput) {
    ...
    let boost = (item.giveEffect && item.giveEffect[npcKey]) ? item.giveEffect[npcKey] : 20;
...
function handleListCitas() {
    let html = `
        <p><strong>🌹 CATÁLOGO DE LUGARES PARA CITAS:</strong></p>
...
function handleInteractionsList() {
    let html = `
        ...
        <table class="cmd-table">
            ...
            <tr><td><code>hablar</code> / <code>conversar</code></td><td><code>hablar kaito</code></td><td>Charla básica.</td></tr>
            ...
            <tr><td><code>cita</code></td><td><code>cita blitzo club_gay_striptease</code></td><td>Salida privada.</td></tr>
...
function handleHelp() {
    let html = `
        <p><strong>🖥️ SISTEMA TERMINAL MULTIVERSAL - COMANDOS PRINCIPALES:</strong></p>
        <table class="cmd-table">
            <tr><th>Comando</th><th>Descripción</th></tr>
            <tr><td><code>mirar</code></td><td>Inspecciona la sala actual.</td></tr>
            <tr><td><code>sims [personaje]</code></td><td>Muestra la ficha tipo Los Sims.</td></tr>
            <tr><td><code>ir [norte/sur/este/oeste]</code></td><td>Navega entre sectores.</td></tr>
            <tr><td><code>cita [personaje] [lugar]</code></td><td>Inicia una cita.</td></tr>
            <tr><td><code>interacciones</code></td><td>Lista completa de acciones sociales.</td></tr>
            <tr><td><code>savegame</code> / <code>loadgame</code></td><td>Guarda o carga la partida.</td></tr>
        </table>
    `;
    printLog(html);
}
